...
export async function handleEditDescriptionEmpresa(id: string, description: string) {
  try {
    const response = await axios.post("https://linksb-production.up.railway.app/editcompany", {
      id: id,
...
export async function handleDeleteLink(IdEmpresa: string) {
  try {
    const response = await axios.post("https://linksb-production.up.railway.app/linksdelete", {
      linkId: IdEmpresa,
    });
...
export async function handleCreatLink(IdEmpresa: string, title: string, logo: string, description: string, url: string, order: number) {
  try {
    const response = await axios.post("https://linksb-production.up.railway.app/links", {
      companyId: IdEmpresa,
      title: title,
      logo: logo,
      description: description,
